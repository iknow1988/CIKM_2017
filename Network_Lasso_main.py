...
def main():
    ...
    lamb = 0
    lambdaMaxValue = 10000
    lambdaUpdateStepSize = 2
    rho = .0001
    dataSize = 10
    type = 1
    ...
    while(lamb <= lambdaMaxValue or lamb == 0):
        ...
        showclusterPerformance(x, False, False, 'train:' + str(trainRMSE) + ' test: ' + str(testRMSE) + ' lamda: ' + str(lamb), "EXP/norm/" + str(lamb) + ".png")
        if(lamb == 0):
            lamb = 2
        else:
